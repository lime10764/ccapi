...
async function handle(req, env, ctx) {
  ...
  if (p === '/admin' || p === '/admin/' || p === '/admin/index.html') {
    if (env.ASSETS) {
      const resp = await env.ASSETS.fetch(new Request(new URL('/admin/index.html', url).href, req)).catch(() => null)
